...
type IndexedAttestationElectra struct {
	AttestingIndices CommitteeIndicesElectra `json:"attesting_indices" yaml:"attesting_indices"`
	Data             phase0.AttestationData  `json:"data" yaml:"data"`
	Signature        common.BLSSignature     `json:"signature" yaml:"signature"`
}

func (p *IndexedAttestationElectra) Deserialize(spec *common.Spec, dr *codec.DecodingReader) error {
	return dr.Container(spec.Wrap(&p.AttestingIndices), &p.Data, &p.Signature)
}

func (a *IndexedAttestationElectra) Serialize(spec *common.Spec, w *codec.EncodingWriter) error {
	return w.Container(spec.Wrap(&a.AttestingIndices), &a.Data, &a.Signature)
}

func (a *IndexedAttestationElectra) ByteLength(spec *common.Spec) uint64 {
	return codec.ContainerLength(spec.Wrap(&a.AttestingIndices), &a.Data, &a.Signature)
}

func (*IndexedAttestationElectra) FixedLength(*common.Spec) uint64 {
	return 0
}

func (p *IndexedAttestationElectra) HashTreeRoot(spec *common.Spec, hFn tree.HashFn) common.Root {
	return hFn.HashTreeRoot(spec.Wrap(&p.AttestingIndices), &p.Data, p.Signature)
}

func IndexedAttestationElectraType(spec *common.Spec) *ContainerTypeDef {
	return ContainerType("IndexedAttestation", []FieldDef{
		{"attesting_indices", spec.CommitteeIndicesElectra()},
		{"data", phase0.AttestationDataType},
		{"signature", common.BLSSignatureType},
	})
}

func ValidateIndexedAttestationIndicesSet(spec *common.Spec, indexedAttestation *IndexedAttestationElectra) (common.ValidatorSet, error) {
	...
	if count := uint64(len(indices)); count > uint64(spec.MAX_VALIDATORS_PER_COMMITTEE_ELECTRA) {
...
func ValidateIndexedAttestationNoSignature(spec *common.Spec, state common.BeaconState, indexedAttestation *IndexedAttestationElectra) error {
...
func ValidateIndexedAttestationSignature(spec *common.Spec, dom common.BLSDomain, pubCache *common.PubkeyCache, indexedAttestation *IndexedAttestationElectra) error {
...
func ValidateIndexedAttestation(spec *common.Spec, epc *common.EpochsContext, state common.BeaconState, indexedAttestation *IndexedAttestationElectra) error {
